...
PSNR1 = psnr(data, image);
fprintf('\n The Peak-SNR value of LCM is %0.4f', PSNR1);
...
for x=1:size(data,1)
    ...
    %% TO move Window to right and bottom, after exceeding the limit 
        for y=1:size(data,2)
        eqdata(x,y)=0;
        if (x>t+limit-1)            %to move rows of window right
            t=t+limit;
            endt=limit+t-1;
        end
        if (y>q+limit-1)             %to move cols of window right 
            q=q+limit;
            endq=limit+q-1;
        end
        if (endt>size(data,1))        %to shift the rows of window to the next line
            % t=t-64;
            endt=size(data,1);
        end
        if (endq>size(data,2))        %to shift the cols of window to the next line
            %  q=q-64;
            endq=size(data,2);
        end
        ...
        for i=t:endt
            for j=q:endq
                ...
                if data(x,y)==data(i,j)
                    eqdata(x,y)=eqdata(x,y)+1;      %storing count of pixels values in matrix eqdata
                end
